...
const {
  consumerKey,
  consumerSecret
} = require("../keys"); // Configure the Twitter strategy for use by Passport.
//
...
var app = express(); // Use application-level middleware for common functionality, including
...
app.use(express.json()); // Initialize Passport and restore authentication state, if any, from the
// session.
...
app.get("/oauth/callback", passport.authenticate("twitter", {
...
app.get("/tweet", isLoggedIn, (req, res) => {
  Tweet.find().populate("createdBy").sort({
    createdAt: "desc"
...
app.post("/tweet/:id", isLoggedIn, (req, res) => {
  Tweet.findByIdAndDelete(req.params.id).then(response => console.log(response)).catch(e => console.error(e));
...
app.put("/tweet/:id", isLoggedIn, (req, res) => {
  ...
  Tweet.findByIdAndUpdate(req.params.id, tweet).then(response => console.log(response)).catch(e => console.error(e));
